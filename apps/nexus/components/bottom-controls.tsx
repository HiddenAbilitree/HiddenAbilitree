...
const getInitialDesktop = () =>
  typeof globalThis.window !== `undefined` && globalThis.matchMedia(`(min-width: 768px)`).matches;
...
export const BottomControls = () => {
  const [smoothScrollEnabled, setSmoothScrollEnabled] = useState(getInitialSmoothScroll);
  ...
  useEffect(() => {
    if (isDesktop) {
      localStorage.setItem(`smooth-scroll`, smoothScrollEnabled ? `true` : `false`);
    }
      ...
      <div className='fixed right-4 bottom-4 z-30 flex flex-col items-end sm:right-auto sm:left-4 sm:z-50 sm:items-start md:bottom-8 md:left-8'>
        ...
        <div className='flex items-center gap-2'>
          <motion.button
            ...
            className='group bg-tns-black hover:bg-tns-black-hover z-10 hidden size-16 items-center justify-center overflow-clip rounded-full border-2 p-3.5 shadow-md hover:cursor-pointer md:flex'
            initial={{ borderColor: `var(--tns-green)` }}
            onClick={() => setSmoothScrollEnabled((cur) => !cur)}
            transition={{ visualDuration: 0.3 }}
            whileHover={{ scale: 1.03 }}
            whileInView={{
              borderColor: smoothScrollEnabled ? `var(--tns-green)` : `var(--tns-red)`,
            }}
            whileTap={{ scale: 0.97 }}
          >
            <Motion className='group-hover:fill-tns-white size-16 fill-[#888888]' />
          </motion.button>

          <motion.button
            aria-label='Chatbot'
            className='group border-tns-blue bg-tns-black hover:bg-tns-black-hover flex size-16 items-center justify-center overflow-clip rounded-full border-2 p-3.5 shadow-md hover:cursor-pointer'
            onClick={() => setChatOpen((cur) => !cur)}
            whileHover={{ scale: 1.03 }}
            whileTap={{ scale: 0.97 }}
          >
            <Robot className='group-hover:fill-tns-white size-16 fill-[#888888]' />
          </motion.button>
        </div>
      </div>
