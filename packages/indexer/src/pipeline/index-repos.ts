...
import { createGitHubGraphQLClient, GitHubApiError } from '@/src/github/graphql-client';
...
export const runIndexPipeline = async (
  ...
  const indexRepo = async (
    repo: GitHubRepo,
    repoOptions: { force: boolean; summariesOnly: boolean },
  ): Promise<IndexResult> => {
    try {
      ...
      if (!repoOptions.force && existing?.last_indexed_sha === currentSha) {
      ...
      const isOwner = repo.owner.login.toLowerCase() === config.github.username.toLowerCase();
